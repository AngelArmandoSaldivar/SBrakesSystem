		...
		case 'listarProductosEdit':
			...
			$consulta="SELECT * FROM articulo ORDER BY stock DESC LIMIT 5";
				...
				if(isset($_POST['productosEdit']))
				{
					...
					m.descripcion LIKE '%".$termino."%' ORDER BY a.stock > 0 DESC, m.descripcion ASC LIMIT 100";
				}
								...
								if($fila["idsucursal"] == $idsucursal) {
									if($fila["stock"] >=1 && $tipo_precio != null) {
										echo "<tr style='color:blue;'>
											...
											<td><p>$ ".$costoMiles."</p></td>
											<td>
												<button style='width: 40px;' type='button' class='btn btn-warning btn-xs' data-dismiss='modal' onclick='agregarDetalle(".$fila["idarticulo"].",\"".$fila["codigo"]."\", \"".$fila["fmsi"]."\", \"".$fila["descripcion"]."\", \"".$fila["marca"]."\", \"".$fila["descripcionMarca"]."\", \"".$fila[$tipo_precio]."\", \"".$fila["stock"]."\", \"".$fila["idsucursal"]."\", \"".$fila["costo"]."\", \"".$fila["publico"]."\", \"".$fila["taller"]."\", \"".$fila["credito_taller"]."\", \"".$fila["mayoreo"]."\" )'><i class='fa fa-plus'></i></button></a>
												<button style='width: 40px;' type='button' class='btn btn-success btn-xs' data-dismiss='modal' onclick='agregarDetalleImagen(".$fila["idarticulo"].",\"".$fila["codigo"]."\", \"".$fila["fmsi"]."\", \"".$fila["descripcion"]."\" , \"".$fila["marca"]."\", \"".$fila["descripcionMarca"]."\", \"".$fila[$tipo_precio]."\", \"".$fila["stock"]."\", \"".$fila["idsucursal"]."\", \"".$fila["costo"]."\", \"".$fila["publico"]."\", \"".$fila["taller"]."\", \"".$fila["credito_taller"]."\", \"".$fila["mayoreo"]."\", \"".$fila["imagen"]."\")'><i class='fa fa-plus'></i></button></a>
											</td>										
										</tr>";
									} else if($fila["stock"] >=1 && $tipo_precio == null){
										$precio = "publico";
